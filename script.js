...
function buyChange() { // eslint-disable-line no-unused-vars
  ...
  const isInCross = bundle.EtheRoox.isInCross(price, 'buy');
  if (isInCross) {
...
function sellChange() { // eslint-disable-line no-unused-vars
  ...
  const total = (amount * price).toFixed(3);
  const isInCross = bundle.EtheRoox.isInCross(price, 'sell');
  if (isInCross) {
...
$(() => {
  $('body').on('click', '#buyCrossSubmit', (e) => {
    ...
    bundle.EtheRoox.trade('buy', JSON.parse($('#buyCrossOrder').val()), $('#buyCrossAmount').val());
  });
});
$(() => {
...
$('#gasPriceModal').on('show.bs.modal', () => {
  $('#gasPrice').val(bundle.EtheRoox.config.ethGasPrice / 1000000000);
});
$(() => {
  $('body').on('click', '#gasPriceSubmit', (e) => {
    e.preventDefault();
    $('#gasPriceModal').modal('hide');
    bundle.EtheRoox.setGasPrice(
      $('#gasPrice').val());
  });
});
